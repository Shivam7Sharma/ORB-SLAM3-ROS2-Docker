...
namespace ORB_SLAM3_Wrapper
{
    ...
    { // set ROS2 logger level to info
        rclcpp::Logger logger = rclcpp::get_logger("rclcpp");
        rclcpp::get_logger("rclcpp").set_level(rclcpp::Logger::Level::Info);
    ...
    void ORBSLAM3Interface::getDirectMapToRobotTF(std_msgs::msg::Header headerToUse, geometry_msgs::msg::TransformStamped &tf)
    {
        ...
        // print to the console using cout the robot frame and the global frame
        std::cout << "Robot frame: " << robotFrame_ << std::endl;
        std::cout << "Global frame: " << globalFrame_ << std::endl;

        if (hasTracked_)
        {
            // print to the console that this function is running
            std::cout << "getDirectMapToRobotTF working" << std::endl;
    ...
    void ORBSLAM3Interface::getMapToOdomTF(const nav_msgs::msg::Odometry::SharedPtr msgOdom, geometry_msgs::msg::TransformStamped &tf)
    {
        // tf.header.stamp;
        tf.header.frame_id = globalFrame_;
        std::cout << "global frame id: " << globalFrame_ << std::endl;
        std::cout << "odom frame id: " << odomFrame_ << std::endl;
    ...
    bool ORBSLAM3Interface::trackStereo(const sensor_msgs::msg::Image::SharedPtr msgLeft, const sensor_msgs::msg::Image::SharedPtr msgRight, Sophus::SE3f &Tcw)
    {
        orbAtlas_ = mSLAM_->GetAtlas();
        ...
        // Track the frame.
        Tcw = mSLAM_->TrackStereo(cvLeft->image, cvRight->image, typeConversions_->stampToSec(msgLeft->header.stamp));
        auto currentTrackingState = mSLAM_->GetTrackingState();
        auto orbLoopClosing = mSLAM_->GetLoopClosing();
